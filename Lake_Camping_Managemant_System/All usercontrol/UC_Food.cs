...
namespace Lake_Camping_Managemant_System.All_usercontrol
{
    public partial class UC_Food : UserControl
    {
        ...
        private void registrer_Click(object sender, EventArgs e)
        {
            ...
            if (isvlid_ID(txtid.Text) && isvlid_type(txtftype.Text) && isvlid_plan(txtplan.Text))
            {
                ...
                con.Open();
                cmd.Connection = con;
                cmd.CommandText = "INSERT INTO food(ID,ftype,plan) VALUES ('" + txtid.Text + "','" + txtftype.Text + "','" + txtplan.Text + "')";
                cmd.ExecuteNonQuery();
                con.Close();
                MessageBox.Show("registered succesful");
                txtid.Clear();
                txtftype.SelectedIndex = -1;
        ...
        public bool isvlid_type(string n)
        {
            ...
            {
                errorProvider1.SetError(this.txtftype, " invalid ");
                return valid;
